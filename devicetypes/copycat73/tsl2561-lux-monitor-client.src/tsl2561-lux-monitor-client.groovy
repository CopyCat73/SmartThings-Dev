...
metadata {
	definition (name: "TSL2561 Lux Monitor Client", namespace: "CopyCat73", author: "Nick Veenstra") {
		capability "Illuminance Measurement"
		capability "Refresh"
       	attribute "infrared", "number"
        ...
        multiAttributeTile(name:"illuminance", type:"generic", width:6, height:4) {
            tileAttribute("device.illuminance", key: "PRIMARY_CONTROL") {
                attributeState "luminosity", label:'${currentValue} lux', defaultState: true, backgroundColors:[
                    ...
                    [value: 1000, color: "#ffffcc"]
                ]
            }
            tileAttribute ("infrared", key: "SECONDARY_CONTROL") {
                attributeState "level", label:'Infrared: ${currentValue} lux', icon: "st.Weather.weather14"
            }
        }
...
def parse(physicalgraph.device.HubResponse hubResponse) {
    ...
    def visible = json.visible
    def infrared = json.infrared
    //log.debug "received values $visible $infrared"
    
    sendEvent([name: "illuminance", value: visible, unit: "lux"])
    sendEvent([name: "infrared", value: infrared, unit: "lux"])
